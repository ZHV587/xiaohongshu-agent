...
function DeepEditor() {
  ...
  const versionScore = (id) => { if (!note.versions) return null; const v = note.versions[id]; return scoreOf(computeChecks({ ...note, title: v.title, body: v.body, tags: v.tags, cover: v.cover })); };
      ...
      <div className="cs" style={{ flex: 1, minWidth: 0, overflowY: "auto", display: "flex", justifyContent: "center", padding: "20px 28px" }}>
        <div style={{ width: 600, maxWidth: "100%", display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                return (
                  <div key={role} style={{ width: 148, flexShrink: 0, display: "flex", flexDirection: "column", gap: 5 }}>
                    <div style={{ position: "relative", width: 148, aspectRatio: "3 / 4", borderRadius: "var(--radius-md)", overflow: "hidden", border: cover ? "2px solid var(--primary)" : "1px solid var(--border)" }}>
                      ...
                      {cover && <textarea value={note.cover} onChange={(e) => actions.updateField("cover", e.target.value)} rows={3} placeholder="封面大字报…" style={{ position: "absolute", top: 10, left: 10, right: 10, border: "none", background: "transparent", resize: "none", color: "#fff", fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 17, lineHeight: 1.15, textShadow: "0 2px 6px rgba(0,0,0,0.55)", outline: "none" }} />}
          ...
          <div style={{ position: "sticky", top: 0, zIndex: 1, display: "flex", flexWrap: "wrap", alignItems: "center", gap: 6, padding: "8px 0", background: "var(--background)", borderBottom: "1px solid var(--border)" }}>
            ...
            {S.quickEmoji.slice(0, 8).map((e) => <button key={e} onClick={() => insertEmoji(e)} disabled={writing} style={{ border: "none", background: "transparent", cursor: "pointer", fontSize: 16, lineHeight: 1, padding: 1, opacity: writing ? 0.4 : 1 }}>{e}</button>)}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, borderTop: "1px solid var(--border)", paddingTop: 14 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              ...
              {S.recommendedTags.filter((t) => !note.tags.includes(t)).slice(0, 6).map((t) => <HashtagTag key={t} addable onAdd={() => actions.addTag(t)}>{t}</HashtagTag>)}
            </div>
          </div>
        </div>
      </div>
...
Object.assign(window, { DeepEditor });
