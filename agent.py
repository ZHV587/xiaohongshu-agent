...
3. 有 emoji 点缀但不过度
4. 标签 5~10 个且与内容相关
5. 使用数据时,选题和文案必须带关键依据摘要及对应 resource_id,不能只声称"来自数据";**例外:双源出选题的线上实时趋势来源在用户采纳前允许只标 note_url、无 resource_id,选题角度里注明"(线上实时:note_url)"即可——不得因此判不合格,也不要为补 resource_id 自动采纳(采纳由用户在面板触发)**
