...
import { Badge, Button, HashtagTag, Icon, Input, Textarea } from "@/components/ds";
import { Eyebrow, PanelHead } from "@/components/studio/ui";
import { useStudio } from "@/components/studio/StudioContext";
import { computeChecks, scoreOf, type CheckResult } from "@/components/studio/rubric";
import { IMAGE_ROLES, QUICK_EMOJI, RECOMMENDED_TAGS, type StudioNote, type VersionId } from "@/components/studio/types";

export function Composer({ layout }: { layout?: string }) {
  const { note, setSection, actions } = useStudio();
  ...
  return (
    <div className="cs" style={{ display: "flex", flexDirection: "column", gap: 14, height: "100%", overflowY: "auto" }}>
      <PanelHead
        ...
        right={
          layout === "deep" ? null : (
            <Button variant="ghost" size="sm" leftIcon={<Icon name="feather" size={13} />} onClick={() => setSection("deep")}>
              深度创作
            </Button>
          )
        }
      ...
      {note.versions && (
        <div style={{ display: "flex", gap: 6 }}>
          {(["A", "B", "C"] as VersionId[]).filter((id) => note.versions?.[id]).map((id) => {
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <span className="font-tabular" style={{ fontSize: 10, color: note.title.length > 20 ? "var(--warning)" : "var(--text-subtle)" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <Eyebrow>
            正文 {writing && <span style={{ color: "var(--primary)", marginLeft: 6 }}>🍠 撰写中…</span>}
          </Eyebrow>
          <div style={{ display: "flex", gap: 5 }}>
            <Button variant="soft" size="sm" leftIcon={<Icon name="sparkles" size={12} />} onClick={actions.polish} disabled={writing}>
              润色
            </Button>
            <Button variant="soft" size="sm" leftIcon={<Icon name="scissors" size={12} />} onClick={actions.shorten} disabled={writing}>
              瘦身
            </Button>
            <Button variant="soft" size="sm" leftIcon={<Icon name="hash" size={12} />} onClick={actions.addTags} disabled={writing}>
              配标签
            </Button>
          </div>
        </div>
          ...
          footer={
            <>
              <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
                {QUICK_EMOJI.slice(0, 10).map((e) => (
                  <button
                    key={e}
                    onClick={() => insertEmoji(e)}
                    disabled={writing}
                    style={{ border: "none", background: "transparent", cursor: "pointer", fontSize: 15, lineHeight: 1, padding: 1, opacity: writing ? 0.4 : 1 }}
                  >
                    {e}
                  </button>
                ))}
              </div>
              <Badge tone={note.body.length > 1000 ? "hot" : "synced"} shape="chip">
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, paddingTop: 2 }}>
          ...
          {RECOMMENDED_TAGS.filter((t) => !note.tags.includes(t))
            .slice(0, 5)
            .map((t) => (
              <HashtagTag key={t} addable onAdd={() => actions.addTag(t)}>
                {t}
              </HashtagTag>
            ))}
        </div>
...
function EmptyComposer() {
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", gap: 12, padding: 24, color: "var(--text-subtle)" }}>
      ...
      <div style={{ fontSize: "var(--text-xs)", maxWidth: 240, lineHeight: "var(--leading-relaxed)" }}>
        点任意一张<b style={{ color: "var(--primary)" }}>选题卡</b>，🍠 会按小红书风格流式生成草稿到这里，然后边改边体检。
      </div>
    </div>
  );
}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <div className="cs" style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 2 }}>
        ...
        <button
          ...
          style={{ width: 80, height: 107, flexShrink: 0, borderRadius: "var(--radius-md)", border: "1px dashed var(--border-strong)", background: "var(--oats-light)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4, cursor: "pointer", color: "var(--text-subtle)" }}
        >
          <Icon name="image-plus" size={16} />
          <span style={{ fontSize: 8 }}>AI 出图</span>
        </button>
      </div>
      <div style={{ display: "flex", gap: 6 }}>
        <Button
          variant="secondary"
          size="sm"
          leftIcon={<Icon name="wand-2" size={13} />}
          onClick={() => actions.updateField("cover", note.versions?.[note.activeVersion]?.cover ?? (note.cover || "爆点\n大字"))}
        >
          换封面文案
        </Button>
        <Button variant="ghost" size="sm" leftIcon={<Icon name="layout-template" size={13} />} onClick={() => actions.toast("🎨 已套用爆款版式（示意）")}>
          套版式
        </Button>
      </div>
    </div>
