...
4. 基于分析,产出 **3~5 个选题方向**,按 system prompt「输出协议」里 `xhs_topics` 代码块的格式输出
...
3. **选题与文案持久化时机（最终回复用户前）**：
   ...
   - 最终回复用户前，文案智能体必须调用 `` `save_generated_copy` `` 并在本地写入，同时调用 `` `sync_copy_to_feishu` `` 同步至飞书多维表格的笔记草稿表中，获取“当前文案 ID”。
