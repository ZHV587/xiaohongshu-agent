...
**质量要求**：
- 无 AI 腔：禁词与表达 DNA 以 `anti-ai-copy-taste` 规约为唯一准绳(本技能不另列禁词)
- 有真实细节（数字、具体场景、实际案例）
- 长短句交错，有节奏感
...
【用户确认持久化规则（极重要）】：
落库与同步遵循主控 system prompt §4《存储路由与权威性》：先写数据库、成功后同步飞书，飞书写经 HITL 人工确认。
...
**绝对不做**：
- 不写「首先…其次…最后…」结构
- 其余禁词/套话以 `anti-ai-copy-taste` 规约为准,不在此重复列举
