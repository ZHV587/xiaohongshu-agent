...
description: |
  【已废弃·不要使用】整篇文案创作已升级为由主控委派 `copywriting-coprocessor` 子代理完成(隔离上下文 + AI 指纹自审 + A/B 多版本),见主控 §2/§3。本技能仅作历史参考保留,主控不要再直调;需要写整篇文案时直接委派子代理。
---
...
| 触发条件 | 推荐 |
|---|---|
| 文案写完需要质检 | 「写完了，做个 AI 味检测和润色。」；内部路由:转入 `xhs-audit` |
