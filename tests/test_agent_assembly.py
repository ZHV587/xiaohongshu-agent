...
除验证 agent 可装配外,钉死收敛期修过的 Critical 安全回归:
...
def test_agent_does_not_register_beta_runtime_rubric(monkeypatch):
    """生产 graph 不接入 DeepAgents beta RubricMiddleware。

    DeepAgents 在 `create_deep_agent` 时会把用户 middleware 并入 compiled
    graph,LangGraph 部署再导入 `agent.py:agent`。因此 beta rubric 不能只靠
    warning 过滤或配置开关规避,必须不进入 middleware 装配。
    """
    import importlib
    ...
    assert not hasattr(agent_module, "rubric_middleware")
    assert not hasattr(agent_module, "content_rubric_activator")
    assert all(type(item).__name__ != "RubricMiddleware" for item in middleware)
    assert all(type(item).__name__ != "ContentRubricActivator" for item in middleware)
...
def test_agent_exposes_shared_model_registry(monkeypatch):
    ...
    assert "rubric" not in status["hot_reload_coverage"]
