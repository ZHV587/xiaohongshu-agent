...
- accounts ←xhs_accounts 档案 + resource_contexts 精确版本归属聚合。
- trends ←暂无真实外部实时趋势数据源 → 真实空集合。
数据不足时一律返回真实空集合([]/0),由前端渲染空态,绝不 mock 或补 0 值占位卡片。

账号维度(account)由 xhs_accounts + resource_contexts 精确绑定到不可变资源版本；未指定
account 时按租户聚合，指定时只能读取对应账号范围。
"""
...
async def internal_studio_analytics(request: Request) -> JSONResponse:
    ...
    try:
        ...
        data = load_analytics(tenant_id=tenant_id, account=account)
    except ValueError as exc:
        return _json_error(400, f"Bad Request: {exc}")
    except PermissionError:
        return _json_error(403, "Forbidden: account is not owned by current user")
...
async def internal_studio_calendar(request: Request) -> JSONResponse:
    ...
    try:
        ...
        data = load_calendar(tenant_id=tenant_id, account=account)
    except ValueError as exc:
        return _json_error(400, f"Bad Request: {exc}")
    except PermissionError:
        return _json_error(403, "Forbidden: account is not owned by current user")
    ...
    return _json_ok(
        {
            "ok": True,
            **load_accounts(
                tenant_id=default_tenant_id(), actor_open_id=actor.open_id
            ),
        }
    )


async def internal_studio_account_upsert(request: Request) -> JSONResponse:
    """创建或更新当前用户的小红书账号档案；不改变生成主流程。"""
    ...
    try:
        ...
        conn = connect()
        try:
            account = AccountRepository(conn).upsert_account(
                tenant_id=default_tenant_id(),
                actor_open_id=actor.open_id,
                account_id=body.get("accountId"),
                platform_account_id=body.get("platformAccountId"),
                display_name=body.get("displayName"),
                niche=body.get("niche"),
                metadata=dict(body.get("metadata") or {}),
                make_default=body.get("makeDefault") is True,
            )
        finally:
            conn.close()
        ...
        logger.warning("studio_account_upsert_failed")
        return _json_error(500, "Account save failed")
    return _json_ok(
        {
            "ok": True,
            "account": {
...
async def internal_studio_pipeline(request: Request) -> JSONResponse:
    ...
    try:
        ...
        queue = load_pipeline(tenant_id=tenant_id, account=account)
    except ValueError as exc:
        return _json_error(400, f"Bad Request: {exc}")
    except PermissionError:
        return _json_error(403, "Forbidden: account is not owned by current user")
...
async def _copy_lifecycle_write(request: Request, action: str) -> JSONResponse:
    ...
    try:
        ...
        account_id = (
            str(body.get("account")).strip()
            if body.get("account") is not None and str(body.get("account")).strip()
            else None
        )
        if account_id is not None and action in {"revision", "adopt"}:
            load_owned_account_context(
                tenant_id=default_tenant_id(),
                actor_open_id=actor.open_id,
                account=account_id,
...
async def internal_studio_schedule(request: Request) -> JSONResponse:
    ...
    try:
        ...
        return _json_error(403, "Forbidden: resource or account is not writable")
...
studio_routes = [
    ...
    Route(
        ...
        internal_studio_account_upsert,
        methods=["POST"],
    ),
