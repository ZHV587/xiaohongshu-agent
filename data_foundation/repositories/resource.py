...
class ResourceRepository(BaseRepository):
            ...
            with connection.transaction():
                ...
                if existing:
                    ...
                    if existing["type"] != resource_type:
                        self._adjust_resource_type_count(connection, tenant_id, existing["type"], -1)
                        self._adjust_resource_type_count(connection, tenant_id, resource_type, 1)
                else:
                    # Insert new resource
                    row = connection.execute(
                        """
    ...
    def _adjust_resource_type_count(
        ...
        connection.execute(
            ...
            set count = greatest(0::bigint, resource_type_counts.count + excluded.count),
                updated_at = now()
            """,
