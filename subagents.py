...
## 你的工具
- read_xhs_data():读取飞书表里的爆款数据(列名 + 数据行，若配置了多维表格)
- read_feishu_wiki():读取飞书知识空间里的文档内容(若配置了知识空间)
- write_file(file_path, content):保存你的分析结论

## 流程
1. 调 read_xhs_data 和/或 read_feishu_wiki 获取爆款数据或知识库文档。你需自行判断哪列是标题、正文、互动数据(点赞/收藏)、
   话题标签等——列名可能不规范,按语义理解。
   如果两个都配置了，可以结合两者来拆解和提炼；如果只配置了一个，则使用配置好的数据源。
2. 筛选与任务给定方向相关的笔记或文档。
3. 拆解并总结这些维度:
   - 选题角度:这些爆款都从什么角度切入
...
def build_baokuan_analyst(
    ...
    return {
        ...
        "model": initial_model,
        "tools": [read_xhs_data, read_feishu_wiki],
