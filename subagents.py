...
- get_resource(resource_id):读取已命中资源的正文、版本和 updated_at
- graph_expand(resource_ids):需要相关案例时做有界关系扩展
- read_xhs_data()/read_feishu_wiki():统一数据底座没有结果时的飞书兜底读取
...
3. 仅当统一检索没有可用结果时,才调用 read_xhs_data 和/或 read_feishu_wiki 兜底。
   你需自行判断标题、正文、互动数据、标签等字段,并明确标注这是兜底来源。
4. 筛选与任务给定方向相关的笔记或文档。
5. 拆解并总结这些维度:
   - 选题角度:这些爆款都从什么角度切入
   ...
   - 话题标签习惯:常用哪些标签、几个
6. 在结论末尾写“关键依据”,逐条记录 resource_id、标题、依据摘要和 updated_at;
   飞书兜底数据没有 resource_id 或 updated_at 时如实标记“未沉淀”或“未知”。
