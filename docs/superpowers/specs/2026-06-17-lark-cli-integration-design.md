...
**身份选择规则(web)**:某请求按当前用户 open_id 取到有效 UAT → 走 user 身份。**取不到 UAT 时不静默退回 bot**,而是返回"请先授权飞书操作权限"提示(加 scope 后老用户 JWT 仍在但无新 UAT,需重新授权一次)。bot 身份只作**显式**选择(模型带 `--as bot`,或本就该用应用身份的操作),不做隐式兜底——避免用户以为在用自己身份、实际用了公共应用身份的越权混淆。

**身份选择规则(CLI)**:CLI 无 server 用户身份,**一律 bot**,不为其保留 user 口子(项目主战场是 web,CLI 仅留作调试)。
...
```
  ...
  用户对话 → JWT(open_id) → auth.py 验签取 identity=open_id
                          ...
                          agent 调 lark 工具 → 身份解析:
                              模型显式 --as bot → 注入 APP_ID/SECRET(bot 身份)
                              否则默认 user:按 open_id 从【UAT 存储层】取 UAT(必要时 refresh 自动刷新)
                                  ├─ 取到 → 注入 LARKSUITE_CLI_USER_ACCESS_TOKEN(user 身份)
                                  └─ 取不到 → 返回"请先授权飞书操作权限"(不静默退回 bot)
                                              │
...
- **接口(抽象,未来换 Postgres 只改实现)**:
  ...
  - `get_uat(open_id) -> UAT | None`:取时若 `expires_at` 临近,用 refresh_token 调飞书刷新、回存、返回新 UAT;刷新失败返回 None(由 lark 工具据此返回"请先授权"提示,不静默退回 bot)。
...
**callback → UAT 存储层的写入通道(已定:方案 a,Python 单写者)**:

callback 是 Next.js 服务端,不能直接写 Python 的加密文件。**否决"两端各自写同一文件"的方案**——web 多用户 + LangGraph server 多并发下,Node(callback 写 A 的新 UAT)与 Python(刷新 B 的 UAT 回写)会同时写一个文件导致损坏,进程内锁挡不住跨进程写竞争。

定方案:Python 后端暴露一个**仅本机、带 HMAC 校验**的内部端点 `POST /_internal/uat`(用 `XHS_JWT_SECRET` 派生密钥签名请求体,防伪造),callback 调它把 UAT 交给 Python。**加密文件只有 Python 一个写者**,并发写竞争收敛到 Python 单进程,进程内锁即足够。TS 侧不碰加密格式,免去跨语言字节级对账。

## 组件 4:后端身份注入(auth.py + agent.py)

- `auth.py` 已能从 JWT 解出 `ctx.user.identity`(= open_id),**无需改鉴权逻辑**。
- agent 侧需把"当前请求的 open_id"传到 lark 工具内部,用于 `get_uat(open_id)`。LangGraph 工具运行时可经 `ToolRuntime`/runtime 上下文拿到当前用户身份(与 backends.py 的 `_user_memory_namespace` 取 `rt.server_info.user.identity` 同源)。

**身份选择规则(web 主场景,修正:不静默退回 bot)**:
- 模型显式带 `--as bot` → 直接 bot 身份(`APP_ID/SECRET`)。用于本就该用应用身份的操作。
- 否则默认 user 身份:解析当前 open_id → `get_uat(open_id)`。
  - 取到 UAT → env 注入 `LARKSUITE_CLI_USER_ACCESS_TOKEN` + `DEFAULT_AS=user`。
  - **取不到 UAT → 不静默退回 bot**,而是返回"请先授权飞书操作权限"提示(老用户 JWT 还在但加 scope 后无新 UAT,需重新登录授权)。理由:web 多用户下,静默用公共 bot 身份替用户执行会造成身份混淆与越权感。bot 只作显式选择,不作隐式兜底。
- **CLI 场景**:无 server 用户身份,**一律 bot**(CLI 仅留作本机调试,不支持 user 身份)。
...
- **情况 E — web 用户无 UAT(未授权 lark 操作)**:用户已登录(JWT 有 open_id)但 `get_uat` 取不到(老用户未走加 scope 的新授权,或刷新失败)。**不静默退回 bot**,返回"请先授权飞书操作权限"提示,引导用户重新登录授权。bot 身份仅在模型显式 `--as bot` 或 CLI 场景使用,不做隐式兜底(避免 web 用户误以为以自己身份操作、实为公共应用身份的越权感)。
...
7. 身份注入:web 有 UAT 走 user 并注入 UAT env;web 无 UAT **不静默走 bot**,返回需授权提示;CLI 无 server 身份直接走 bot 注入 APP_ID/SECRET;模型显式 `--as bot` 时走 bot。
...
| UAT 过期致命令失败 | refresh_token 自动刷新;失败时提示重登(web 不静默退 bot) |
...
| 多用户 token 经 keychain 串号 | 验证 lark-cli env 模式纯用环境变量、不读写 OS keychain(命门,实现前必查) |
| UAT 文件并发写损坏 | 定方案 a:加密文件只 Python 单写者,callback 经内部端点提交,进程内锁 |
...
4. web `login`/`callback` 改造(加 scope + callback 经内部端点 `POST /_internal/uat` 提交 UAT)
...
4. 实现期必查(多用户隔离命门):验证 lark-cli 以 `LARKSUITE_CLI_USER_ACCESS_TOKEN` env 注入运行时**只用 env、不读也不写 OS keychain**。若它会落 keychain,多用户同机跑会串身份,须改用每请求独立 `LARKSUITE_CLI_CONFIG_DIR` 隔离。
