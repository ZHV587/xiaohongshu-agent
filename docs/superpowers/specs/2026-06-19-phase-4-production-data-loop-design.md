...
- 日期：2026-06-19
- 状态：设计稿，Phase 4.0 官方扩展边界收敛、Phase 4.1 同步与状态闭环、Phase 4.2 检索增强文案、Phase 4.3 创作记忆、Phase 4.4 效果反馈均已实现
...
5. Agent 只能通过 LangChain tools 或 MCP tools 访问数据能力，不能直接执行 SQL、Cypher 或 lark-cli。
6. 飞书 CLI 只作为内部适配器，被后端服务或工具封装调用，不暴露给用户，也不成为业务入口。
7. 运维脚本只允许用于部署、健康检查、备份、回滚等平台操作，不允许承载业务同步或 Agent 行为。
8. 高风险写操作继续走 DeepAgents `interrupt_on` 或明确的人类确认；使用 `interrupt_on` 的运行路径必须有 LangGraph/DeepAgents checkpointer 支撑。
...
2. 使用 `tools=[...]` 注册普通 LangChain tools 或 MCP tools。
3. 使用 `subagents` 保持多 Agent 协作。
4. 使用 `middleware` 做模型路由、质量检查、重试等横切能力。
5. 使用 `backend` / memory / checkpointer 保存会话和 Agent 状态。
6. 使用 `interrupt_on` 做需要人工确认的动作边界，并确保对应运行路径有 checkpointer。
7. LangChain tools 使用 `RunnableConfig` 读取当前 Web 用户身份和租户上下文；MCP tools 必须通过 adapter/interceptor 传入等价的受控身份上下文。
8. 使用 LangGraph server + Web 前端作为运行入口。

Postgres、`sync_runs`、outbox worker、定时同步、embedding worker、资源图谱、创作沉淀和效果反馈不是 DeepAgents 官方内置模块，也不需要伪装成 DeepAgents 内核能力。它们属于应用服务层。它们通过稳定的服务函数、LangChain tools 或 MCP tools 暴露给 DeepAgents，保持在官方扩展点之内。
...
| 能力 | 官方扩展点 | 设计约束 |
| --- | --- | --- |
| 数据检索 | `create_deep_agent(tools=[...])` + LangChain tools 或 MCP tools | Agent 只调用 `search_resources`、`semantic_search_resources`、`graph_expand`、`get_resource` 等工具，不直连数据库 |
| 手动同步 | LangChain tool 或 MCP tool + 用户上下文 | Web 对话触发 `sync_feishu_resources`，工具内部调用应用服务层，不出现业务 CLI |
| 数据状态查询 | LangChain tool 或 MCP tool | `get_data_foundation_status` 返回结构化状态，LLM 负责解释给用户 |
...
| 人工确认 | `interrupt_on` + checkpointer | 发布、外部写入、批量覆盖等高风险动作必须可中断确认；无 checkpointer 的路径不能承诺 HITL |
| 会话和长期状态 | `backend` / memory / checkpointer | 会话状态、文件状态和长期记忆走官方后端接口，不自建旁路会话机制 |
| 权限身份 | `RunnableConfig` 或 MCP adapter/interceptor 等价身份上下文 | 工具读取当前用户、租户、飞书身份，权限在工具/服务层过滤 |
...
现有代码里仍有几条历史路径虽然可以依托官方扩展方式，但身份、权限、确认或生命周期边界还没有完全收敛。第四阶段实施时必须一起补齐：

| 当前路径 | 问题 | 改造目标 |
| --- | --- | --- |
| `lark_mcp_server.py::execute_lark_command` | MCP 本身是官方支持的 tool 接入方式；当前问题是这层包装没有贯通当前 Web 用户身份、权限审计和确认语义 | 可以继续保留 MCP，但必须通过 `tool_interceptors` 或等价 adapter 机制证明 MCP tool 能拿到当前用户身份，并在写操作上接入 `interrupt_on`；若当前 MCP adapter 无法安全透传身份，再改为直接 LangChain tool |
| `web/src/app/api/feishu/sync/route.ts` -> `tools/web_bridge_runner.py --action sync` | 前端按钮直连 Python runner 写飞书 Base，绕过 DeepAgents tool、middleware 和 `interrupt_on` | 改为 `save_generated_copy` / `sync_copy_to_feishu` LangChain tool 或 MCP tool，由 Web 对话触发；写飞书前走 `interrupt_on` |
| `web/src/app/api/feishu/notify/route.ts` -> `tools/web_bridge_runner.py --action notify` | 前端按钮直连 Python runner 发飞书消息，绕过 Agent 官方工具链和人工确认 | 改为 `send_review_notification` LangChain tool 或 MCP tool，由 Agent 调用；发送前走 `interrupt_on` |
| `tools/web_bridge_runner.py` 多 action runner | 不是交互式业务 CLI，但承载了业务动作、配置动作、认证动作，边界混杂 | 只保留认证/配置桥接等平台动作；业务动作迁移到 Agent-facing LangChain tools 或 MCP tools，底层再调用应用服务层 |
| `agent.py::get_lark_mcp_tools()` 启动时同步拉 MCP tools | MCP 是官方扩展点；当前风险是 import 阶段同步取工具导致生命周期和错误边界不清 | 保留 MCP 路线，但要补启动失败降级、身份透传和测试覆盖；不能因为 MCP 是官方方式就跳过这些边界 |
...
实现状态：已完成。主智能体和 `topic-content` skill 采用统一检索优先契约，`baokuan-analyst` 通过官方 subagent tools 使用同一组检索能力，RubricMiddleware 检查来源依据、源端更新时间 `source_updated_at`、本地索引时间 `indexed_at` 和无依据断言。结构化选题与文案均携带关键依据摘要并由 Web 卡片展示；语义检索失败时保留关键词检索，完全无可用来源时明确提示“当前数据不足”并建议同步或补充数据。内容创作不再把 `read_xhs_data` 或 `read_feishu_wiki` 作为未沉淀兜底；飞书读取只在 `sync_feishu_resources` 的服务层 loader 内部使用，先沉淀到 Postgres 再作为证据检索。
...
实现状态：已完成。新增 `save_generated_topic`、`save_generated_copy`、`save_user_feedback` 三个 LangChain tools，通过 DeepAgents 官方 `tools` 扩展点暴露给 Web 对话 Agent。生成选题以 `generated_topic` 资源沉淀，生成文案以 `generated_copy` 资源沉淀，用户反馈与修改意见以 `user_feedback` 或 `revision_request` 资源沉淀；生成资源通过 `derived_from` 边连接实际 evidence 来源，反馈通过 `feedback_on` 边连接目标内容。所有写入仍经过 Postgres repository、tenant/actor 上下文和 outbox，不引入业务 CLI 或管理后台。
...
实现状态：已完成。新增 `save_performance_metric` 和 `get_resource_performance` 两个 LangChain tools，通过官方 `tools` 扩展点接入 Agent。发布后的点赞、收藏、评论、转发、浏览和转化会沉淀为 `performance_metric` 资源，并通过 `measured_by` 边连接对应内容资源。第一版采用可解释评分公式 `(likes + 2*collects + 3*comments + 4*shares + 5*conversions) / views`，无浏览量时不除以 views；不引入预测模型。Agent 在回答“过去表现如何”“为什么推荐”时必须读取这些工具数据。
...
Phase 4.5 经实现前自审拆分为四个连续子阶段：

1. Phase 4.5A 运行状态可信化：重写多租户 scheduler/outbox，加入真实 embedding processor、租约恢复、重试死信、模型原子切换和运行事实。
2. Phase 4.5B 内部健康接口：通过 LangGraph 官方 `http.app` custom route 暴露只读、脱敏的内部健康数据。
3. Phase 4.5C 管理员状态界面：集成到现有 Web 对话视图，只允许管理员查看，支持全局汇总、租户筛选和自动刷新。
4. Phase 4.5D 正式生产运行时：停止将 `langgraph dev` 作为服务器生产运行时，迁移到 LangGraph 正式部署方式。

Phase 4.5A 详细设计见 `docs/superpowers/specs/2026-06-20-phase-4-5a-operational-truth-design.md`。当前旧 scheduler 会静默吞异常，旧 outbox 仅校验 topic 就标记成功，不能作为健康状态依据；实施 4.5A 时必须整体删除并替换，不保留兼容路径。管理员界面必须排在可信运行事实和内部健康接口之后。
...
3. Tool 测试：LangChain tools 的 `RunnableConfig` 身份解析、MCP tools 的等价身份传递、权限过滤、错误返回。
...
5. Deep Agents 官方支持 human-in-the-loop，在关键工具调用前暂停等待批准；官方要求 HITL 路径有 checkpointer，以便中断后恢复。
6. LangChain MCP adapter 官方支持 `MultiServerMCPClient(..., tool_interceptors=[...])`，interceptor 可以访问 runtime context，并把用户身份或凭证注入 MCP tool call。
7. 本地安装的 `deepagents 0.6.10` 中，`create_deep_agent` 签名包含 `tools`、`middleware`、`subagents`、`backend`、`interrupt_on`、`checkpointer`、`store` 等参数；本地 `langchain_mcp_adapters` 的 `MultiServerMCPClient` 签名包含 `tool_interceptors`。
