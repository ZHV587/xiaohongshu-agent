...
| 当前路径 | 问题 | 改造目标 |
| --- | --- | --- |
| `lark_mcp_server.py::execute_lark_command` | MCP 本身是官方支持的 tool 接入方式；当前问题是这层包装没有贯通当前 Web 用户身份、权限审计和确认语义 | 可以继续保留 MCP，但必须证明 MCP tool 能拿到或等价接收当前用户身份，并在写操作上接入 `interrupt_on`；若做不到，再改为直接 LangChain tool |
