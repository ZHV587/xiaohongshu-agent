...
7. LangChain tools 使用 `RunnableConfig` 读取当前 Web 用户身份和租户上下文；MCP tools 必须通过 adapter 传入等价的受控身份上下文。
...
| 权限身份 | `RunnableConfig` 或 MCP adapter 等价身份上下文 | 工具读取当前用户、租户、飞书身份，权限在工具/服务层过滤 |
...
| 当前路径 | 问题 | 改造目标 |
| --- | --- | --- |
| `lark_mcp_server.py::execute_lark_command` | MCP 本身是官方支持的 tool 接入方式；当前问题是这层包装没有贯通当前 Web 用户身份、权限审计和确认语义 | 可以继续保留 MCP，但必须证明 MCP tool 能拿到或等价接收当前用户身份，并在写操作上接入 `interrupt_on`；若当前 MCP adapter 无法安全透传身份，再改为直接 LangChain tool |
