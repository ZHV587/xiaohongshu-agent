...
**核心对称性**:两个新 processor 完全复刻 `EmbeddingProcessor` 的 `Processor` 协议(`topic` / `state()` / `process()`),注册进 `default_processor_registry`。scheduler 对所有 active topic 一视同仁地派发。**待验证假设(阶段 2 开头先确认)**:scheduler `_prepare_processors` 的 `unblock_available` 与 `process_outbox_batch` 对非 embedding topic 同样生效(embedding topic 有额外的 reconcile 分支,meili/graph 无此分支,需确认普通 topic 路径完整跑通)。若 scheduler 对普通 topic 有遗漏,则在阶段 2 一并补;预期不用改。
