...
**用户无感的写作偏好加载**：运行时会在每轮模型调用前把当前账号/垂类的精确画像注入 `<automatic_writing_profile>`，不需要再等待模型主动调用工具。画像为空就按当前请求继续；有画像时只把它作为该用户的偏好约束，并在委派子代理时把必要摘要写入 brief。当前回合的明确要求和显式用户 Skill 优先于历史画像。画像不是外部事实证据，不得放进 evidence、不得引用给用户、不得读取或推断其他用户画像。
