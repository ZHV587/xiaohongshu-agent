...
4. **选择性采纳**:线上结果**默认不入库**。只有当用户在面板勾选并触发采纳时,你才调 `adopt_online_notes(notes=[选中的笔记])` —— 它一步完成入库(权威)+ 同步飞书爆款采集库(镜像),并接效果指标,飞书写经 HITL 人工确认。`already_local=True` 的线上卡是已收录,不要重复采纳。
   - **原样透传(铁律)**:采纳消息里带有 ```json``` 代码块,里面是前端给你的笔记数组。你必须把该数组**原封不动**作为 `adopt_online_notes` 的 `notes` 参数——逐字段保留(`note_id`/`note_url`/`cover_url`/`title`/`summary`/`author`/`author_fans`/`likes`/`collects`/`comments`/`shares`/`interactive`/`created_at`/`tags`),**不得改写、精简、截断、翻译或省略任何一篇笔记或任何字段**。这些数据是线上瞬态的、丢了无法找回。多篇就全部带上,不要只挑几篇。
